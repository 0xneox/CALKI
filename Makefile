...
docker:
	mkdir -p /tmp/calki/build
	cp Dockerfile-build /tmp/calki/build/Dockerfile
	docker build -t calki/build /tmp/calki/build
	docker run -ti -v ${PWD}:/sources -u calki calki/build bash -c  "make setup; make release"
	...
	cp Dockerfile-run admintool/release/Dockerfile; docker build -t calki/run admintool/release

...
if [ "${bootstrap}" = "bootstrap" ] ; then
    ...
    sudo docker build -f Dockerfile-build -t zibbit/calki-build .
    sudo docker build -f Dockerfile-build-speedup -t zibbit/calki-build .
fi

echo "2) build target/install using image calki-build"
cd ${SOURCE_DIR}
home=$HOME

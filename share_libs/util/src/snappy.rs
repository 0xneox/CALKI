...
const CALKI_COMPRESS_SIZE: usize = 40 * 1024;
pub fn calki_compresse(input: Vec<u8>) -> Vec<u8> {
    if input.len() > CALKI_COMPRESS_SIZE { compress(&input) } else { input }
}

pub fn calki_decompress(input: Vec<u8>) -> Vec<u8> {

...
pub trait BaseHandler {
    fn select_topic(method: &String) -> String {
        let topic = if method.starts_with("calki_send") {
                        "jsonrpc.new_tx"
                    } else if method.starts_with("calki") || method.starts_with("eth") {
                        "jsonrpc.request"
...
mod test {
    ...
    fn test_get_topic() {
        ...
        assert_eq!(Handler::select_topic(&"calki_send".to_string()), "jsonrpc.new_tx".to_string());
        assert_eq!(Handler::select_topic(&"calki".to_string()), "jsonrpc.request".to_string());

...
pub mod method {
    pub const CALKI_BLOCK_BUMBER: &'static str = "calki_blockNumber";
    pub const CALKI_GET_BLOCK_BY_HASH: &'static str = "calki_getBlockByHash";
    pub const CALKI_GET_BLOCK_BY_NUMBER: &'static str = "calki_getBlockByNumber";
    pub const CALKI_GET_TRANSACTION: &'static str = "calki_getTransaction";
    pub const CALKI_SEND_TRANSACTION: &'static str = "calki_sendTransaction";
...
impl MethodHandler {
    ...
    pub fn from_req(&self, rpc: RpcRequest) -> Result<reqlib::Request, Error> {
        match rpc.method.as_str() {
            method::CALKI_BLOCK_BUMBER => {
                self.block_number(rpc)
            }
            method::NET_PEER_COUNT => {
                self.peer_count(rpc)
            }
            method::CALKI_GET_BLOCK_BY_HASH => {
                self.get_block_by_hash(rpc)
            }
            method::CALKI_GET_BLOCK_BY_NUMBER => {
                self.get_block_by_number(rpc)
            }
            method::CALKI_GET_TRANSACTION => {
...
//After this test, into a separate file.
#[cfg(test)]
mod tests {
    use super::*;
    use Id;
    ...
    fn test_rpc_serialize() {
        let rpc_body = r#"{"jsonrpc":"2.0","method":"calki_blockNumber","params":[],"id":"1"}"#;
        ...
        assert_eq!(rpc.method, "calki_blockNumber".to_string());
    ...
    fn test_rpc_deserialize() {
        let rpc = RpcRequest {
            ...
            method: method::CALKI_BLOCK_BUMBER.to_owned(),
            id: Id::Str("2".to_string()),
            params: Params::Array(vec![]),
        };

        let rpc_body = serde_json::to_string(&rpc).unwrap();
        assert_eq!(rpc_body, r#"{"jsonrpc":"2.0","method":"calki_blockNumber","id":"2","params":[]}"#);
    }
    ...
    #[test]
    fn test_calki_send_transaction() {
        ...
        let rpc1 = RpcRequest {
            jsonrpc: Some(Version::V2),
            method: method::CALKI_SEND_TRANSACTION.to_owned(),
            id: Id::Str("2".to_string()),
    ...
    #[test]
    //After the transaction adds valid_until_block, it is compatible with testing previous transactions.
    fn test_blocklimit_backword_compatibility() {
        let rpc = r#"{"jsonrpc":"2.0","method":"calki_sendTransaction","params":[""],"id":2}"#;
        let request: RpcRequest = serde_json::from_str(rpc).unwrap();
        let params: Result<(String,), Error> = request.params.parse();
        assert!(params.is_ok());
    }
    ...
    #[test]
    fn calki_get_log_deserialization() {
